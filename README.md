...
`release-check` 是发布前的 fail-closed 总门禁。它不会发布或修改外部平台，只接受私密 evidence，并要求每个 `--require` 平台记录都是 `PASS`，且项目、源码提交、候选 SHA-256 和 probe 类型完全一致：管理器只能是明确的 `stage-b-manager` 版本 probe，设备只能是 `android-emulator-manager` 或 `oneplus-15-firefox-manager`，GitHub 只能是 `github-publish` 或已核对的 `github-publish-adapter`，Greasy Fork 只能是 `greasyfork-first-import` 或 `greasyfork-version-sync`。缺少、过期、类型错误或 `BLOCKED` 的证据都会阻止后续发布。
