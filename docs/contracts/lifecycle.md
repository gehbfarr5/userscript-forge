...
- 项目 `targets.requiredVerification` 中映射到管理器、Android 模拟器、一加 15、GitHub 或 Greasy Fork 的声明，会自动成为 `release-check` 的必需 kind；调用者不能通过省略该 kind 让发布门禁变宽。
- 项目配置中的 GitHub 仓库和 `greasyForkRequired` 也会自动要求相应公开平台 evidence；发布命令不能只验证本机或设备后跳过公开发布核对。
